...
private[sql] trait RowValueReader extends SettingsAware {
  ...
  def rowColumns(currentField: String): Seq[String] = {
    rowColumnsMap.get(currentField) match {
      case Some(v) => v
      case None => throw new EsHadoopIllegalStateException(s"Field '$currentField' not found + in " + rowColumnsMap.keySet + " ; typically " +
        "this occurs with arrays which are not mapped as single value")
//      case None => Seq("")
    }
  }
